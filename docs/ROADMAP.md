...
## v1.0.x — Phase 1 patches (current track)
...
- Search: tag/genre filters and sort options (the `SearchQuery` + `SearchSort` plumbing
  already exists), surfacing of MangaDex genre shortcut tags.
- Bookmarks UI on top of the existing `bookmarks` table/DAO.
- Unit tests for `MangaDexMapper` (normalization, chapter de-dup) and repository caching.
- Accessibility, string extraction/localization, and dependency bumps.

## v1.1.0 — Phase 2 (multi-provider)

The architecture was built for this; **only the seams below need implementation**.
