...
- **Subsequent updates to Phase 1 are released as PATCH versions** (`1.0.1`, `1.0.2`, …):
  bug fixes, performance and UI polish, dependency bumps, and small refinements that do
  not change the public app behavior in a breaking way.
- The next **MINOR** version (`1.1.0`) is reserved for **Phase 2** — multi-provider support
  (additional sources, result merging, fallback, source priority, deduplication). See
  [ROADMAP.md](ROADMAP.md).
- `versionName` in `app/build.gradle.kts` is the source of truth and currently reads
  `1.0.0` (`versionCode = 1`). Bump the patch digit and `versionCode` together for each
  patch release.
...
First release. A local-first manga reader for Android with MangaDex as the only source,
built on a clean, provider-pluggable architecture.
...
**Source layer (MangaDex)**
- `MangaProvider` abstraction with a single `MangaDexProvider` implementation.
- Retrofit + OkHttp client with timeouts, polite `User-Agent`, transient-failure retry,
  and kotlinx-serialization; DTO→domain mappers ported from the reference POC.
